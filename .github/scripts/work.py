...
def card(p, x, y):
    s = [
        ...
        f'  <rect x="{x + 20}" y="{y + 24}" width="2" height="20" rx="1" fill="{ACC}"/>',
        f'  <text x="{x + 34}" y="{y + 40}" font-family="{SANS}" font-size="16.5" font-weight="600" '
        f'letter-spacing="-0.2" fill="{TEXT}">{esc(p["name"])}</text>',
        f'  <text x="{x + CARD_W - 20}" y="{y + 39}" text-anchor="end" font-family="{MONO}" font-size="9.5" '
        f'letter-spacing="1.3" fill="{ACC}" opacity="0.85">{esc(p["focus"].upper())}</text>',
        f'  <path d="M{x + 20} {y + 56}H{x + CARD_W - 20}" stroke="{LINE}"/>',
    ]

    for i, line in enumerate(wrap(p["why"], 62)):
        s.append(
            f'  <text x="{x + 20}" y="{y + 80 + i * 19}" font-family="{SANS}" font-size="12.5" '
            f'fill="{MUTED}">{esc(line)}</text>'
        )

    cx = x + 20
    for label in p["stack"]:
        cw = round(len(label) * 6.4 + 22)
        s.append(
            f'  <rect x="{cx}" y="{y + CARD_H - 44}" width="{cw}" height="23" rx="6" '
            f'fill="#0B0F16" stroke="{LINE}"/>'
        )
        s.append(
            f'  <text x="{cx + cw / 2:.0f}" y="{y + CARD_H - 28}" text-anchor="middle" font-family="{MONO}" '
            f'font-size="10.5" fill="{CHIP}">{esc(label)}</text>'
        )
        cx += cw + 7

    if p.get("live") or p.get("url"):
        s.append(
            f'  <text x="{x + CARD_W - 20}" y="{y + CARD_H - 28}" text-anchor="end" font-family="{MONO}" '
            f'font-size="9.5" letter-spacing="1.2" fill="{DIM}">LIVE</text>'
        )
    return s
...
def render(projects):
    ...
    s = [
        ...
        f'  <text x="{PAD}" y="40" font-family="{MONO}" font-size="10.5" letter-spacing="1.6" '
        f'fill="{DIM}">SELECTED.WORK {"-" * 52}</text>',
        f'  <text x="{W - PAD}" y="40" text-anchor="end" font-family="{MONO}" font-size="10.5" '
